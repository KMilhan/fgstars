...
QJsonObject SchedulerJob::toJson() const
{
    ...
    return
    {
        ...
        {"startupTime", startupTime.isValid() ? startupTime.toString(Qt::ISODate) : "--"},
        {"completionTime", finishAtTime.isValid() ? finishAtTime.toString(Qt::ISODate) : "--"},
        {"stopTime", stopTime.isValid() ? stopTime.toString(Qt::ISODate) : "--"},
        {"altitude", alt},
