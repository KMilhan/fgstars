...
class dms
{
    public:
        ...
        /** @return angle in degrees expressed as a double.
            	*/
        inline const double &Degrees() const
        {
            return D;
        }
        ...
        {
            return int(reduce().Degrees() / 15.0);
        }
        ...
        /** @return angle in hours expressed as a double in the range -11.999 to 0 to 12.0
             ...
             * or hours/minutes/seconds.  An hour is equal to 15 degrees.
             */
        inline double HoursHa() const
        {
            return Hours() <= 12.0 ? Hours() : Hours() - 24.0;
        }
        ...
        /** @return a nicely-formatted string representation of the angle
             ...
             */
        const QString toDMSString(const bool forceSign = false, const bool machineReadable = false,
                                  const bool highPrecision = false, char machineReadableSeparator = ':') const;
        ...
        /** @return a nicely-formatted string representation of the angle
             ...
             * @param highPrecision adds milliseconds, if @c false the seconds will be shown as an integer
             */
        const QString toHMSString(const bool machineReadable = false, const bool highPrecision = false,
 ...
 */
inline dms operator""_deg(long double x)
{
    return dms(double(x));
}
 ...
 */
inline dms operator""_h(long double x)
{
    return dms(double(x * 15.0));
}

/**
 * Create a constant angle in radians
 */
inline dms operator""_rad(long double x)
{
    return dms(double(x / dms::DegToRad));
}

/**
 * Create a constant angle from a DMS string
 */
inline dms operator""_dms(const char *dmsString)
{
    return dms::fromString(QString(dmsString), true);
}

/**
 * Create a constant angle from a HMS string
 */
inline dms operator""_hms(const char *hmsString)
{
    return dms::fromString(QString(hmsString), false);
}
